...
AC_ARG_ENABLE(
    ...
    if test "$APXS_BIN" = "no"; then
		...
		AP_PROG_PATH="${AP_SBINDIR}/`$APXS_BIN -q progname 2>/dev/null`"
        ...
        if test "x$enable_apachetest" = "xyes" ; then
            if test "$min_apache20_version" != "no"; then
                ...
                if test ! -x $APR_CONFIG; then
                    APR_CONFIG="`$APXS_BIN -q APR_BINDIR 2>/dev/null`/apr-2-config"
                fi
				if test ! -x $APR_CONFIG; then
				...
				if test -x $APU_CONFIG; then
					...
					APU_LDFLAGS=`$APU_CONFIG --libtool-libtool 2>/dev/null`
				else
					APU_CONFIG=""
                fi
                ...
                TEST_APACHE_VERSION(20,$min_apache20_version,
                    AC_MSG_RESULT(yes)
					AP_MAJOR_VERSION="2"
		    		AP_MINOR_VERSION="`$AP_PROG_PATH  -V | sed 's/@<:@://@:>@/ /g' | awk '/version/ { print @S|@4 }' | awk -F. '{ print(@S|@2) }' 2> /dev/null`"
					AP_VERSION="${AP_MAJOR_VERSION}.${AP_MINOR_VERSION}"
                    AC_DEFINE(AP_MAJOR_VERSION,2,[Apache HTTPD Major Version Number])
                    AC_DEFINE_UNQUOTED(AP_MINOR_VERSION,$AP_MINOR_VERSION,[Apache HTTPD Minor Version Number])
                    APXS_EXTENSION=.la
