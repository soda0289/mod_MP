...
*/
/*
static int find_table_by_id(db_table_t** element, apr_array_header_t*array, const char* id){
	int i;
	for(i = 0;i < array->nelts;i++){
		if(apr_strnatcasecmp(APR_ARRAY_IDX(array, i, db_table_t).id,id)==0){
			*element = &(APR_ARRAY_IDX(array, i, db_table_t));
			return 0;
		}
	}
	return -1;
...
static int find_column_by_id(db_table_column_t** element, apr_array_header_t*array, const char* id){
	int i;
	for(i = 0;i < array->nelts;i++){
		if(apr_strnatcasecmp(APR_ARRAY_IDX(array, i, db_table_column_t).id,id)==0){
			*element = &(APR_ARRAY_IDX(array, i, db_table_column_t));
			return 0;
		}
	}
	return -1;
...
*/
/*
tatic int find_app_by_id(app_list_t* app_list,const char* id,app_t** app){
	app_node_t* app_node;
...
int find_select_column_from_query_by_table_id_and_query_id(db_table_column_t** select_column,db_query_t* query, const char* table_id, const char* column_id){
	...
	for(column_index = 0; column_index < query->select_columns->nelts;column_index++){
		column = ((db_table_column_t**)query->select_columns->elts)[column_index];
...
int generate_app_queries(apr_pool_t* pool, db_queries_t* db_queries,apr_xml_elem* queries, db_params_t* db_params, error_messages_t* error_messages){
	...
	//As of now the app XML file contains a database element with child query elements
	for(query_elem=queries->first_child;query_elem != NULL; query_elem = query_elem->next){
		if(apr_strnatcmp(query_elem->name,"query") == 0){
			...
			query->select_columns = apr_array_make(pool,10,sizeof(db_table_column_t*));
			query->tables = apr_array_make(pool,10,sizeof(db_table_t*));
			...
			for(query_child_elem = query_elem->first_child;query_child_elem != NULL; query_child_elem =query_child_elem->next){
				if(apr_strnatcmp(query_child_elem->name,"table") == 0){
					...
					table_ptr = (db_table_t**)apr_array_push(query->tables);
					*table_ptr = table;
					...
					for(col_elem = table_elem->first_child;col_elem != NULL;col_elem = col_elem->next){
						if(apr_strnatcmp(col_elem->name,"column") == 0){
							...
							column_ptr = (db_table_column_t**)apr_array_push(query->select_columns);
							*column_ptr = column;
					...
					for(count_child_elem = query_child_elem->first_child;count_child_elem != NULL; count_child_elem = count_child_elem->next){
						if(apr_strnatcmp(count_child_elem->name,"table") == 0){
							...
							column = apr_pcalloc(pool, sizeof(db_table_column_t));
