...
decoding_job_t* get_decoding_job_queue(decoding_queue_t* decoding_queue, error_messages_t* error_messages){
	...
	if(decoding_queue->queue->waiting){
		...
		if(decoding_queue->queue->head < 0 || decoding_queue->queue->head > 63 ){
			error_messages_add(decoding_queue->error_messages,ERROR,"Errroroororor", "head is less than 0");
		}
...
int add_decoding_job_queue(decoding_job_t* job,decoding_queue_t* decoding_queue){
	...
	if(!free){
		error_messages_add(decoding_queue->error_messages,ERROR,"Error adding to queue","Queue is FULL!");
		return -1;
	}
...
int does_decoding_job_exsits(apr_pool_t* pool,decoding_queue_t* decoding_queue,decoding_job_t** dec_job){
	...
	if(decoding_queue->queue->waiting & decoding_queue->queue->working){
		error_messages_add(decoding_queue->error_messages,ERROR,"Working and waiting are set WTF MAN",apr_strerror(rv, error_message,512));
	}
	...
	if(found > 1){
		error_messages_add(decoding_queue->error_messages, ERROR, "Error found duplicate", "found duplicate in working/waiting queue");
	}
