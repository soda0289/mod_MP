...
int init_music_query(apr_pool_t* pool,apr_array_header_t* db_queries, apr_xml_elem* xml_params, const void** global_context, error_messages_t* error_messages){
	int status;
	...
	//Init Music Globals
	*global_context = music_globals = apr_pcalloc(pool, sizeof(music_globals_t));

	music_globals->pool = pool;

	music_globals->db_queries = db_queries;
...
int reattach_music_query(apr_pool_t* child_pool, error_messages_t* error_messages,const void* global_context){
	...
	for(i = 0; i < music_globals->db_queries->nelts; i++){
		db_queries_t* db_queries = &(((db_queries_t*)music_globals->db_queries->elts)[i]);
		db_params_t* db_params = db_queries->db_params;

		//Setup Database Connection
		rv = connect_database(child_pool, db_params);
		if(rv != APR_SUCCESS){
			...
			//Prepare database queries for app usage
			status = prepare_database(db_params->db_config);
			if(status != 0){
				dbd_error = apr_dbd_error(db_params->db_config->dbd_driver,db_params->db_config->dbd_handle, status);
				add_error_list(music_globals->error_messages, ERROR, "Database error couldn't prepare",dbd_error);
			}
			apr_pool_cleanup_register(child_pool, db_params->db_config,(void*) close_database	, apr_pool_cleanup_null);
		}
	}
...
int get_music_query(music_query_t* music_query){
	...
	if(query_nouns == NULL || query_nouns[1] == NULL){
		//Not enough info
		add_error_list(music_query->error_messages, ERROR, error_header, "Query doesn't contain enough words.");
		return -3;
	}
	...
	if (apr_strnatcasecmp(query_nouns[1], "songs") == 0){
		...
		//Invlaid type
		add_error_list(music_query->error_messages, ERROR, error_header, apr_pstrcat(music_query->pool,"Query type ",query_nouns[1]," is not supported by API.",NULL));
		return -2;
	...
	if(status != 0){
		add_error_list(music_query->error_messages, ERROR, error_header, apr_pstrcat(music_query->pool, "Query type ",query_nouns[1]," is not supported by Database.", NULL ));
		return -1;
	}
	...
	init_query_parameters(music_query->pool,&(music_query->query_parameters));
	//Copy array
...
int output_db_query_json(music_query_t* query){
	...
	copy_error_messages(query->error_messages, query->db_query->db_params->db_config->database_errors, query->pool);
...
int run_music_query(input_t* input, output_t* output, const void* global_context){
	...
	error_num = run_query(music_query->db_query,music_query->query_parameters, &(music_query->results));
	if(error_num != 0 && music_query->results != NULL){
		...
		switch(music_query->type){
			...
			case TRANSCODE:{
				...
				if(error_num != 0){
					add_error_list(music_query->error_messages, ERROR, "Error transcoding audio", apr_itoa(music_query->pool, error_num));
					output_status_json(output);
				}
				break;
			}
